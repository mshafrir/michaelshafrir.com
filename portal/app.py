...
YAHOO_BOSS_API_KEY = "kZuLGaPV34H862VltztBe9b3iP4WYtHnIsTKLIYHYa9KIcaFrUswGhpJjYvRCiIzP7o-"
...
def yahoo_boss_lookup(count, page):
    ...
    if offset + count >= 1000:
        ...
        if portal_search_response and portal_search_response.status_code == 200:
            boss_json = simplejson.loads(portal_search_response.content)
...
def google_lookup(count, page):
    ...
    portal_urls = []
    if portal_search_response and portal_search_response.status_code == 200:
        google_json = simplejson.loads(portal_search_response.content)
...
def bing_lookup(count, page):
    ...
    if portal_search_response and portal_search_response.status_code == 200:
        ...
        bing_json = simplejson.loads(portal_search_response.content)
...
class DataDump(webapp.RequestHandler):    
    def get(self):
        self.response.headers['Content-Type'] = 'text/plain'
        self.response.out.write(simplejson.dumps([portal.props() for portal in PortalStats.all()]))
...
class DataLoad(webapp.RequestHandler):
    def get(self):
        ...
        if data_dump_response and data_dump_response.status_code == 200:
            for portal in simplejson.loads(data_dump_response.content):
